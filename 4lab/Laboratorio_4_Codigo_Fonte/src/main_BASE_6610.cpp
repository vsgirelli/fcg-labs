...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Guardamos matriz model atual na pilha
        PushMatrix(model);
            // Definimos Translação, Rotações, e Escala da cabeça
            model = model * Matrix_Translate(0.0f, 0.05f, 0.0f);
            model = model * Matrix_Rotate_Z(3.141592f);
            ...
            // Enviamos matriz model atual para a GPU
            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            // Desenhamos a cabeça
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Scale(0.2f, 0.1f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com o escalamento da mão direita
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                ...
                PushMatrix(model);
                    ...
                    PushMatrix(model);
                        ...
                        PushMatrix(model);
                            ...
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                            DrawCube(render_as_black_uniform); // #### PE DIREITO
                        PopMatrix(model);
                    PopMatrix(model);
                PopMatrix(model);
            PopMatrix(model);
