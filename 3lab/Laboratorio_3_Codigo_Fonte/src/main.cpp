...
// Variáeis que controlam a rotação da cabeça
float g_HeadAngleX = 3.14;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00261595 - Valéria S. Girelli", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Multiplica a matriz do modelo pela matriz de translação considerando as
        // posições iniciais do torso no global
        ...
        PushMatrix(model); // push pra desenhar a cabeça
          ...
          model = model
            ...
            * Matrix_Rotate_X(g_HeadAngleX - g_AngleX); // Rotação inicial da cabeça pra deixar ela virada pra cima

            PushMatrix(model);
              model = model * Matrix_Scale(0.3f, 0.3f, 0.3f); // escala pra diminuir e fazer a cabeça
              glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                // já que tem que fazer o resto sobre ela
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model);
                      ...
                      PushMatrix(model);
                        model = model * Matrix_Scale(0.2f, 0.1f, 0.2f); // escala pra mão direita
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
